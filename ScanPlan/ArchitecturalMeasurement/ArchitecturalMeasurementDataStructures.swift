...
// MARK: - Core Architectural Parameters

/// Comprehensive parametric data for a room with professional accuracy
struct ArchitecturalParameters: Identifiable, Codable {
    let id = UUID()
    let walls: [WallParameters]
    let openings: [OpeningParameters]
    let floorArea: Float
    ...
    var openingCount: Int { openings.count }
    var perimeter: Float { walls.reduce(0) { $0 + $1.length } }
    var averageWallHeight: Float {
        guard !walls.isEmpty else { return 0 }
        return walls.reduce(0) { $0 + $1.height } / Float(walls.count)
    }

    static func empty() -> ArchitecturalParameters {
        return ArchitecturalParameters(
            ...
            ceilingHeight: 0.0,
            volume: 0.0,
            roomBounds: RoomBounds.empty(),
            measurementTimestamp: Date()
        )
    }
}

/// Detailed parameters for individual walls
struct WallParameters: Identifiable, Codable {
    let id = UUID()
    let wallId: UUID
    let length: Float
    let height: Float
    ...
    var area: Float { length * height }
    var netArea: Float {
        let openingArea = openings.reduce(0) { $0 + $1.area }
        return max(0, area - openingArea)
    }
}

/// Room bounds in 3D space
struct RoomBounds: Codable {
    let min: simd_float3
    let max: simd_float3
    let center: simd_float3
    let size: simd_float3

    static func empty() -> RoomBounds {
        return RoomBounds(
...
/// Building-level parametric data
struct BuildingParametrics: Codable {
    let rooms: [ArchitecturalParameters]
    let totalFloorArea: Float
    let totalVolume: Float
    let buildingBounds: BuildingBounds
    let analysisTimestamp: Date

    var roomCount: Int { rooms.count }
}

/// Building bounds in 3D space
struct BuildingBounds: Codable {
    let min: simd_float3
    let max: simd_float3
    let center: simd_float3
    let size: simd_float3

    static func empty() -> BuildingBounds {
...
enum MeasurementQuality: String, CaseIterable, Codable {
...
enum ProfessionalStandard: String, CaseIterable {
    ...
    var requiredAccuracy: Float {
        switch self {
        case .architectural: return 0.005
        case .construction: return 0.010
        case .survey: return 0.002
        case .asBuilt: return 0.003
        case .conceptual: return 0.020
        }
    }
    ...
    var displayName: String {
        ...
        case .survey: return "Survey Grade"
        case .asBuilt: return "As-Built"
        case .conceptual: return "Conceptual"
        }
    }
}

// MARK: - Room Classification Results (from original file)
